...
export default class KnnField {
	...
	getNNearestNeighbour(vector: KnnVector, n: number = this.knnVectors.length, distanceFunction: DistanceFunction = this.distanceFunction): KnnVector[] {
		let vectorsCopy: Array<KnnVector> = [...this.knnVectors];
		vectorsCopy =vectorsCopy.sort((vectorA: KnnVector, vectorB: KnnVector) => {
			let distA = distanceFunction(vector, vectorA)
			let distB = distanceFunction(vector, vectorB)
			return distA - distB
		})
		vectorsCopy =vectorsCopy.filter((v)=>{
			return v.id !== vector.id
		})
